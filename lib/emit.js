...
async function connect({ server, room, name = 'emitter', timeoutMs = HANDSHAKE_TIMEOUT_MS } = {}) {
  ...
  transport.send({
    ...
    extensions: [],
    // Omit `room` entirely when the caller named none. Defaulting it to
    // 'default' made silence into a POSITIVE claim of the public square, and a
    // receiver in any named room is required to close on a mismatched claim
    // (§5.8) — so the documented default of this API was precisely the value
    // refused by every named room, and the caller saw only "closed before
    // handshake" with no mention of rooms. Absence is not a claim and is
    // admitted; `connect({ room: 'default' })` still claims it, on purpose.
    ...(room === undefined || room === null || room === '' ? {} : { room }),
