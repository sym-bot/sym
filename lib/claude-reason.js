...
function invoke(opts) {
  ...
  try {
    ...
    let execBin = claudeBin;
    let execArgs = args;
    if (isWin && claudeBin.endsWith('.cmd')) {
      const cliJs = path.join(path.dirname(claudeBin), 'node_modules', '@anthropic-ai', 'claude-code', 'cli.js');
      if (fs.existsSync(cliJs)) {
        execBin = process.execPath; // node.exe
        execArgs = [cliJs, ...args];
      }
    }
    ...
    const result = execFileSync(execBin, execArgs, execOpts).trim();
