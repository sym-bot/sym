...
function invoke(opts) {
  ...
  // Use execFileSync with argument array — no shell interpolation, no escaping issues.
  // System prompt passed as --append-system-prompt argument directly.
  const claudeBin = process.env.CLAUDE_BIN || '/Users/hongwei/.local/bin/claude';
  ...
  try {
    ...
    if (!cat7) throw new Error('No valid CAT7 JSON in Claude output');
    return cat7;
  } catch (err) {
