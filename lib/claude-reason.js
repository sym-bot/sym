...
function invoke(opts) {
  ...
  // On Windows (or long prompts), prepend system prompt to stdin
  // instead of --append-system-prompt CLI arg (8192 char limit on Windows).
  const useStdinSystem = isWin || rolePrompt.length > 4000;
  const args = [...claude.prefixArgs, '-p', '--output-format', 'json', '--model', model,
  ...
  try {
    ...
    const result = execFileSync(claude.bin, args, {
      input: prompt,
      encoding: 'utf8',
      timeout: 120000,
