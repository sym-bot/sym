...
const { encode, DIM, processHeuristicSVAF, computeFieldVerdicts, decryptFields, mintRemixKey, resolveTetherAnchor, signTetherAttestation, verifyTetherAttestation, recomputeKey } = require('@sym-bot/core');
...
class FrameHandler {
  ...
  _rejectOnBadSignature(peerId, peerName, msg) {
    ...
    // Audience (§18.3.1): a genuinely-signed cmb1- CMB whose bound group is not this
    ...
    if (!aud.ok) {
      this._node._log(`[sym-security] ${aud.reason} on CMB ${String(cmb.key || '').slice(0, 16)} from ${peerName} — rejected`);
      this._node.emit('metric', { type: 'cmb-audience-rejected', from: peerName, key: cmb.key, reason: aud.reason });
      return true;
    }
  ...
  _handleCmbFetchResult(peerId, peerName, msg) {
    ...
    if (!cmb || cmb.key !== pending.key) { pending.miss(peerId); return; }
    const recomputed = typeof recomputeKey === 'function' ? recomputeKey(cmb) : null;
  ...
  _processNeuralSVAF(result, msg, peerName, peerId, originTs, now) {
    ...
    // `mintRemixKey` hashes over fields + parent(s) + receiverName under the
    // active scheme (cmb1-/legacy), giving (i) idempotent dedup for retries,
    // (ii) distinct remix keys across different receivers fusing the same
    // input, (iii) remix key ≠ parent key by construction.
    if (incomingKey && fusedEntry.cmb) {
      const newKey = mintRemixKey(fusedEntry.cmb.fields, [incomingKey], this._node.name);
