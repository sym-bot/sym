...
class FrameHandler {
   ...
   * @private
   */
  _surfaceDirectedReject(msg, peerName, peerId, now) {
    ...
    this._node._log(`Directed CMB from ${peerName} (peer-bound, MMP §4.4.4) — SVAF rejected for memory; surfacing to agent anyway`);
  ...
  _processNeuralSVAF(result, msg, peerName, peerId, originTs, now) {
    ...
    if (decision === 'rejected') {
      ...
      this._surfaceDirectedReject(msg, peerName, peerId, now);
