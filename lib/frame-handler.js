...
const { recordKey, recordLineage, recordParents, setRecordLineage } = require('./record');
...
const { encode, DIM, processHeuristicSVAF, computeCategoryVerdicts, decryptCategories, blockKeyV2, resolveTetherAnchor, signTetherAttestation, verifyTetherAttestation, recomputeKey, classifyAddress } = require('@sym-bot/core');
...
class FrameHandler {
    ...
    if (!v.valid && !grandfathered) {
      ...
      if (author && author !== peerName) {
        ...
        if (authorKey) {
          ...
          if (va.valid) { msg._cmbVerified = true; return false; }
  ...
  _processNeuralSVAF(result, msg, peerName, peerId, originTs, now) {
    ...
    const incomingKey = recordKey(msg.cmb) || msg.key;
    const incomingAncestors = recordLineage(msg.cmb)?.ancestors || [];
    ...
    //
    // This used to mint a remix key over categories + parents + the RECEIVER'S NAME, which
    ...
    if (incomingKey && fusedEntry.cmb) {
      const fusedKey = blockKeyV2(fusedEntry.cmb.categories);
  ...
  async _processHeuristicSVAF(msg, peerName, peerId, originTs, now, ageSeconds) {
    ...
    const result = await processHeuristicSVAF({
      tetherAnchor: tetherAnchor && tetherAnchor.categories ? tetherAnchor : undefined,
      msg,
      peerName,
      ...
      config: {
        ...
        categoryWeights: this._node._svafFieldWeights,
    ...
    if (result.tether?.checked && !result.tether.tethered && recordLineage(result.fusedEntry?.cmb)) {
      const departedFrom = recordParents(result.fusedEntry.cmb)[0] ?? null;
      setRecordLineage(result.fusedEntry.cmb, null); // fresh root — must clear the section the record ACTUALLY carries
      ...
      this._node.emit('metric', { type: 'lineage-tether-severed', key: recordKey(result.fusedEntry.cmb), anchor: result.tether.anchorKey, drift: result.tether.drift });
    ...
    if (result.tether?.checked && result.fusedEntry?.cmb
      ...
      try {
        result.fusedEntry.cmb.tether = signTetherAttestation({
          of: recordKey(result.fusedEntry.cmb),
