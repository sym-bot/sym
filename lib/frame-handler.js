...
const { encode, DIM, processHeuristicSVAF, computeFieldVerdicts, decryptFields, mintRemixKey, resolveTetherAnchor } = require('@sym-bot/core');
  ...
  async _processHeuristicSVAF(msg, peerName, peerId, originTs, now, ageSeconds) {
    ...
    if (result.tether?.checked && !result.tether.tethered && result.fusedEntry?.cmb?.lineage) {
      ...
      result.fusedEntry.cmb.provenance = {
        ...result.fusedEntry.cmb.provenance,
        tether: { severed: true, anchor: result.tether.anchorKey, drift: result.tether.drift, departedFrom },
      };
      ...
      result.fusedEntry.cmb.provenance = {
        ...result.fusedEntry.cmb.provenance,
        tether: { severed: false, anchor: result.tether.anchorKey, drift: result.tether.drift },
      };
    }
