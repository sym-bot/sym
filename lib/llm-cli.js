...
const { resolveClaudeCLI, isWin } = require('./platform');
...
function callClaudeCLI(config, systemPrompt, userPrompt) {
  return new Promise((resolve, reject) => {
    ...
    // Pass the user prompt via stdin instead of as a positional arg.
    // Windows command line max is ~32KB; large prompts (research arXiv
    // batches, big RSS digests) trigger spawn ENAMETOOLONG when passed
    // as args. Stdin has no length limit. Claude CLI accepts stdin
    // input when --print/-p is set and no positional prompt is given.

    // Resolve claude binary properly on Windows: use node + cli.js directly
    // to bypass the .cmd shim that spawns visible cmd.exe windows.
    ...
    // Write the user prompt to stdin and close it so claude knows the
    // input is complete.
    proc.stdin.on('error', (err) => {
      // Ignore EPIPE if claude has already exited
    });
    ...
    proc.on('error', (err) => {
      clearTimeout(timer);
      if (err.code === 'ENOENT') {
        reject(new Error(`claude CLI not found. Install Claude Code: https://docs.claude.com/claude-code (resolved to: ${bin})`));
      } else {
    ...
    proc.on('close', (code) => {
      ...
      try {
        ...
        // Normalize usage categories to match the OpenAI/Anthropic shape that
