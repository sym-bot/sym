  ...
  for (const cmd of candidates) {
    try {
      ...
      if (match && parseInt(match[1]) >= 3 && parseInt(match[2]) >= 8) {
        ...
        try {
          const fullPath = execSync(whichCmd, { encoding: 'utf8', timeout: 5000 }).trim().split('\n')[0];
...
function resolveClaudeCLI() {
  ...
  if (!claudePath) {
    try {
      ...
      claudePath = execSync(cmd, { encoding: 'utf8', timeout: 5000 }).trim().split('\n')[0];
    } catch {
  ...
  if (isWin && claudePath.endsWith('.cmd')) {
