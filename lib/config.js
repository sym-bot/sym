...
function resolveAvailableName(base, { maxSuffix = 64 } = {}) {
  ...
  for (let i = 0; i < maxSuffix; i++) {
    ...
    const lock = readLockFile(path.join(nodeDir(candidate), 'lock.pid'));
    if (lock && lock.pid !== process.pid && lockIsHeldByLiveProcess(lock)) {
...
function acquireIdentityLock(name, _attempt = 0) {
  ...
  if (lock) {
    ...
    if (lockIsHeldByLiveProcess(lock)) {
      const err = new Error(
        `Identity '${name}' is already locked by PID ${lock.pid ?? 'unknown'}. ` +
