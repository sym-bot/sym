...
function createCMB(opts = {}) {
  ...
  // MMP v2.0 emitter flip (reader-first §migration): when v2.0 emission is enabled, populate
  // the signed-preimage fields the published mmp-sig-v2.0 suite covers — addressScheme,
  // signatureSuite, createdByNodeId, application. Their PRESENCE is what makes signCMB sign under
  // the v2.0 preimage, so this is the flip itself and MUST stay gated behind the caller until the
  // v2.0 READER is deployed across the mesh — a peer still on the old verifier rejects a v2.0
  // record. assertionId is NOT set here: it is derived from the full preimage by the signing
  // layer (this module cannot import cmb-signing without a cycle), so the emit path sets it after.
  //
  // FAIL CLOSED: createdByNodeId is the signed node identity (distinct from createdBy, the agent
  // id). v2.0 emission without it would mint a record the suite refuses to sign, so require it.
  if (opts.emitV2 === true) {
    if (typeof opts.createdByNodeId !== 'string' || opts.createdByNodeId.length === 0) {
      throw new Error('createCMB: emitV2 requires createdByNodeId — the signed node identity (mmp-sig-v2.0)');
    }
    metadata.addressScheme = 'mmp-cmb-merkle-v2';
    metadata.signatureSuite = 'mmp-sig-v2.0';
    metadata.createdByNodeId = opts.createdByNodeId;
    metadata.application = opts.application ?? null;
  }
