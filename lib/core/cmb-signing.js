...
// ── MMP v2.0 published contract (mmp-sig-v2.0) ────────────────────────────────────────────
// The published, executable MMP 2.0 signing suite (meshcognition.org/spec/mmp). It corrects
// the internal mmp-sig-v2 in three ways the audit found: the preimage binds the protocol
// version and address scheme, a signed author NODE ID (not just a display name), and a signed
// application-section commitment — so an actionable payload can no longer be altered under a
// valid signature. The content-address (blockKeyV2) is UNCHANGED, so this is a signatures-only
// migration: readers accept v2.0 first, emitters flip last.
const MMP_PROTOCOL_VERSION = '2.0';
const MMP_ADDRESS_SCHEME = 'mmp-cmb-merkle-v2';
const MMP_SIGNATURE_SUITE = 'mmp-sig-v2.0';

/**
 * applicationCommitmentV1 (mmp-app-v1) — the signed commitment to an optional actionable
 * application section. Absent → a fixed lp('0') sentinel so "no application" is distinguishable
 * from any present one. Present → the section's mediaType, schema, encoding, byteLength and
 * content digest, netstring-framed. The bytes travel; the SIGNATURE commits to their digest, so
 * a relay cannot alter the payload without invalidating the record. Byte-exact with the
 * published contract's scripts/mmp/lib.mjs.
 */
function applicationCommitmentV1(application) {
  const domain = Buffer.from('mmp-app-v1\n', 'utf8');
  if (application == null) {
    return crypto.createHash('sha256').update(Buffer.concat([domain, lp('0')])).digest('hex');
  }
  const bytes = Buffer.from(application.data, 'base64url');
  if (application.encoding !== 'base64url' || application.data.includes('=')) {
    throw new Error('mmp-app-v1: application data must be unpadded base64url');
  }
  if (bytes.toString('base64url') !== application.data) throw new Error('mmp-app-v1: application data is not canonical base64url');
  if (bytes.length !== application.byteLength) throw new Error('mmp-app-v1: application byteLength mismatch');
  if (bytes.length > 524288) throw new Error('mmp-app-v1: application exceeds 524288 bytes');
  if (`sha256-${crypto.createHash('sha256').update(bytes).digest('hex')}` !== application.digest) {
    throw new Error('mmp-app-v1: application digest mismatch');
  }
  if (!/^[a-z0-9][a-z0-9!#$&^_.+/-]*$/.test(application.mediaType) || application.mediaType.includes(';')) {
    throw new Error('mmp-app-v1: mediaType must be canonical lowercase ASCII without parameters');
  }
  const schema = String(application.schema ?? '').normalize('NFC');
  return crypto.createHash('sha256').update(Buffer.concat([
    domain, lp('1'), lp(application.mediaType), lp(schema),
    lp('base64url'), lp(decimal(application.byteLength)), lp(application.digest),
  ])).digest('hex');
}

/**
 * The published v2.0 signing payload, byte-exact with meshcognition.org's mmp-sig-v2.0. Same
 * framing rule as v2 (domain tag raw + trailing newline, everything after uniformly lp()); adds
 * protocolVersion, addressScheme, createdByNodeId (signed identity), and the application
 * commitment. Requires the record to carry signatureSuite/addressScheme/createdByNodeId — a
 * record missing them is not a v2.0 record.
 */
function signingPayloadV2_0(cmb) {
  if (!cmb || !cmb.metadata) throw new Error('mmp-sig-v2.0: refusing to sign a record with no metadata');
  const m = cmb.metadata;
  if (m.signatureSuite !== MMP_SIGNATURE_SUITE) throw new Error(`mmp-sig-v2.0: signatureSuite must be ${MMP_SIGNATURE_SUITE}`);
  if (m.addressScheme !== MMP_ADDRESS_SCHEME) throw new Error(`mmp-sig-v2.0: addressScheme must be ${MMP_ADDRESS_SCHEME}`);
  if (typeof m.key !== 'string' || !m.key) throw new Error('mmp-sig-v2.0: refusing to sign a record with no key');
  if (keyScheme(m.key) !== 'v1') throw new Error('mmp-sig-v2.0: refusing a non-v1 key');
  if (typeof m.createdByNodeId !== 'string' || !m.createdByNodeId) throw new Error('mmp-sig-v2.0: createdByNodeId is required — authorship is a signed identity, not a display name');
  if (typeof m.createdBy !== 'string' || !m.createdBy) throw new Error('mmp-sig-v2.0: createdBy is required');
  if (m.lineage != null && m.lineage.parents != null && !Array.isArray(m.lineage.parents)) {
    throw new Error('mmp-sig-v2.0: lineage.parents must be an array when present');
  }
  const parents = sortedBytewise(m.lineage?.parents ?? []);
  return Buffer.concat([
    Buffer.from('mmp-sig-v2.0\n', 'utf8'),
    lp(MMP_PROTOCOL_VERSION),
    lp(MMP_ADDRESS_SCHEME),
    lp(m.key),
...
/** assertionId — names WHICH v2.0 assertion (asrt-<sha256(preimage)>). Two records with the
 *  same cognition key but different application sections have DIFFERENT assertion ids. */
function assertionIdV2_0(cmb) {
  return `asrt-${crypto.createHash('sha256').update(signingPayloadV2_0(cmb)).digest('hex')}`;
}
...
function signCMB(cmb, privateKeyB64url) {
  ...
  if (cmb && cmb.metadata) {
    // Sign under the published v2.0 suite when the record declares it (emitter flip); otherwise
    // the internal v2 preimage, unchanged, so nothing that emits v2 today changes until the
    // encoder populates the v2.0 fields.
    const payload = cmb.metadata.signatureSuite === MMP_SIGNATURE_SUITE ? signingPayloadV2_0(cmb) : signingPayloadV2(cmb);
    const sig = crypto.sign(null, payload, privateKeyObject(privateKeyB64url));
...
function verifyCMB(cmb, publicKeyB64url) {
  ...
  if (cmb && cmb.metadata) {
    ...
    try {
      // Reader-first v2.0 migration: a record that declares the published suite is verified
      // against the published preimage; everything else stays on the internal v2 preimage. The
      // suite tag in the SIGNED metadata selects the construction, so a downgrade to the weaker
      // preimage cannot be forced by a relay — signatureSuite is itself covered by the v2.0
      // signature it selects.
      const payload = m.signatureSuite === MMP_SIGNATURE_SUITE ? signingPayloadV2_0(cmb) : signingPayloadV2(cmb);
      const ok = crypto.verify(null, payload, publicKeyObject(publicKeyB64url), Buffer.from(m.sig, 'base64url'));
...
module.exports = { signingPayloadV2, signingPayloadV2_0, applicationCommitmentV1, assertionIdV2_0, categoryParentsCommitment, sigDigestV2, signCMB, verifyCMB, checkAudience, privateKeyObject, publicKeyObject };
