 ...
 * grantee's announced key, the owner, and both timestamps — so one signature can
 * not be replayed into a different room, onto a different node, with a substituted
 * key, or with a stretched expiry. Both signer and verifier use this function.
 * @param {object} g - { room, grantee, granteeKey, grantedBy, grantedAt, expiresAt }
 * @returns {Buffer}
 */
function roomGrantPayload(g) {
  return Buffer.from(
    `room-join|${g.room}|${g.grantee}|${g.granteeKey || ''}|${g.grantedBy}|${g.grantedAt}|${g.expiresAt}`,
    'utf8',
  );
}
