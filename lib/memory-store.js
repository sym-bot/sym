...
class MemoryStore {
  ...
  _expandLineage(lineage) {
    ...
    // Ancestors = the transitive closure (MMP §15.2). SEED with the chain carried
    // on the wire (`lineage.ancestors`): the receiver stores its own remix, never
    // the incoming CMB, so the incoming parent is absent from the local index —
    // recomputing from the index alone would drop the ROOT at ≥2 hops and break
    // offline-remix detection. Then union any locally-known ancestors of the
    // parents. Ancestors are advisory (provenance, not authority) and the whole
    // CMB is bounded by MAX_FRAME_SIZE, so preserving the wire chain is safe.
    const ancestorSet = new Set(Array.isArray(lineage.ancestors) ? lineage.ancestors : []);
