...
class MemoryStore {
  ...
  _cmbKey(cmb, content) {
    ...
    // Content-addressable: cmb- + first 32 hex chars of SHA256(field texts).
    // MUST stay in sync with @sym-bot/core cmb-encoder.js and the Swift SDK
    // (sym-core-swift CMBEncoder.sha256Prefix). Cross-SDK CMB dedup depends on
    // identical algorithm + truncation length on every node.
    if (cmb?.fields) {
      const fieldTexts = ['focus', 'issue', 'intent', 'motivation', 'commitment', 'perspective']
        .map(f => cmb.fields[f]?.text || '')
        .join('|') + '|' + (cmb.fields.mood?.text || '');
      return 'cmb-' + crypto.createHash('sha256').update(fieldTexts).digest('hex').slice(0, 32);
    }
