...
class MemoryStore {
  ...
  _cmbKey(cmb, content) {
    ...
    // minter, which the cmb-only cutover DELETED — so this path threw for every fields-bearing
    ...
    if (cmb?.fields) {
      return blockKeyV2(cmb.fields);
    }

    // Fallback for a content-only entry with no CAT7 fields at all. Deliberately NOT a CMB
