...
class MemoryStore {
  ...
  _cmbKey(cmb, content) {
    ...
    // Delegate to @sym-bot/core for cross-SDK consistency — one source of truth:
    // cmbKeyV1() in sym-core/lib/cmb-encoder.js, mirrored by sym-core-swift. Post
    // cmb--only cutover (§19.1) this is the v1 content address (bare cmb-<64hex>);
    // the legacy 32-hex cmbKey() is deleted.
    const { cmbKeyV1 } = require('@sym-bot/core');
    if (cmb?.fields) {
      return cmbKeyV1(cmb.fields);
    }

    // Fallback for a keyless, fields-less block: hash raw content. Distinct input
    // space from the field-keyed path (cannot collide), and now the FULL 64-hex
    // digest so the derived key is a valid v1 form (bare cmb-<64hex>) — a truncated
    // 32-hex key would fail-closed reject at core's dispatch post-cutover.
    return 'cmb-' + crypto.createHash('sha256').update(content || '').digest('hex');
  }
