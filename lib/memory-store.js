...
class MemoryStore {
  ...
  write(content, opts = {}) {
    ...
    const hashFile = path.join(dir, `cmb-${hash}.json`);
    if (fs.existsSync(hashFile)) return null;

    const now = Date.now();
    const cmbKey = opts.cmb?.key || `cmb-${hash}`;
    const entry = {
      key: cmbKey,
      content,
      source: this._source,
    ...
    try {
      ...
      console.error(`[SYM] memory-store: write failed: ${e.message}`);
      return null;
    }
    return entry;
  }
  ...
  receiveFromPeer(peerId, entry) {
    ...
    ensureDir(dir);
    const filename = `${entry.timestamp || Date.now()}-${Math.random().toString(36).slice(2, 6)}.json`;
    try {
      fs.writeFileSync(path.join(dir, filename), JSON.stringify(entry, null, 2));
    } catch (e) {
      console.error(`[SYM] memory-store: peer write failed: ${e.message}`);
    }
  }
  ...
  recentCMBs(limit = 5) {
    ...
    return entries.map(e => {
      ...
      return createCMB({ rawText: e.content, createdBy: e.source || this._source });
    });
  }
