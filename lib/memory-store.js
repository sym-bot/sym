...
class MemoryStore {
  ...
  _cmbKey(cmb, content) {
    ...
    // Delegate to @sym-bot/core for cross-SDK consistency. Previously this
    // re-implemented the same SHA256-truncated key logic, which drifted
    // multiple times. Now there is one source of truth: cmbKey() in
    // sym-core/lib/cmb-encoder.js, mirrored by sym-core-swift's
    // CMBEncoder.sha256Prefix. See MMP Section 8.2.
    const { cmbKey } = require('@sym-bot/core');
    if (cmb?.fields) {
      return cmbKey(cmb.fields);
    }

    // Fallback: hash raw content directly. Distinct input space from the
    // field-keyed path above, so cannot collide. Same SHA256+truncate(32)
    // transform to match the primary path's algorithm and key length.
    return 'cmb-' + crypto.createHash('sha256').update(content || '').digest('hex').slice(0, 32);
  }
