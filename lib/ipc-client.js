...
class SymDaemonClient extends EventEmitter {
  ...
  /**
   * Store a memory with structured CAT7 categories via daemon.
  ...
  remember(categories, opts = {}) {
    // parents carry the lineage edge (each { key }) so an emitted CMB can declare its
    // source; the daemon forwards them to node.remember (MMP §14 remix lineage).
    this._send({ type: 'remember', categories, tags: opts.tags, parents: opts.parents });
