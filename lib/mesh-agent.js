...
class MeshAgent {
  ...
  async start() {
    ...
    // Catchup: daemon broadcasts "catchup" message to trigger immediate domain poll
    node.on('message', ({ content }) => {
      if (content === 'catchup') {
  ...
  async _onCMBAccepted(entry) {
    ...
    const stored = this._node.remember(remixFields, { parents: [parent] });
    if (!stored) {
  ...
  async _checkDomain() {
    ...
    const stored = this._node.remember(fields);
    if (!stored) {
