...
const { recordCreatedBy, recordLineage } = require('./record');
...
// Load ~/.sym/relay.env at module load time so per-agent env vars
// (SYM_RESEARCH_PROVIDER, SYM_COO_MODEL, etc.) are available when
// agents read them at the top of agent.js — before MeshAgent ctor.
(function loadRelayEnv() {
  if (process.env.SYM_RELAY_URL) return;
...
// ── Founder Action Detection ────────────────────────────

/**
 * Auto-detect whether a CMB warrants founder attention.
 *
 * Used as a fallback when the agent's LLM didn't explicitly set
 * `_meta.founderAction`. Scans intent + issue + commitment + mood.text
 * for action-signal vocabulary. The wider category surface catches
 * high-quality CMBs that put the action signal in the commitment
 * or mood category rather than restating it in intent.
 *
 * The vocabulary list mixes legacy hedge-paraphrase trigger words
 * (kept for backwards compatibility) with the more specific verbs
 * that disciplined extraction prompts produce (fetch, draft, cite,
 * arxiv, endorser, submission, etc.). Without the second category,
 * a research CMB that says "Fetch full PDF today. Flag endorser
 * candidates..." would fail to promote even though it's exactly the
 * kind of dated, actionable signal the founder needs to see.
 *
 * See MMP Section 11: Feedback Modulation.
 */
function detectFounderAction(categories) {
  const get = (k) => {
  ...
  const actionSignals = [
    // Legacy hedge-paraphrase triggers
    'founder', 'human', 'decision', 'approve', 'prioritize', 'prioritise',
    'strategy', 'strategic', 'outreach', 'partnership', 'acquire', 'acquisition',
    'budget', 'hire', 'resource', 'launch', 'ship', 'release', 'publish',
    'pitch', 'investor', 'demo', 'contact', 'introduce', 'introduction',
    'competitive', 'threat', 'urgent', 'urgency', 'blocker', 'blocked',
    'increase', 'decrease', 'change direction', 'pivot',
    // Concrete action verbs from disciplined extraction prompts
    'fetch', 'pull', 'check', 'flag', 'draft', 'reach out', 'email',
    'reply', 'respond', 'submit', 'submission', 'cite', 'citation',
    'review', 'merge', 'tag', 'release', 'announce', 'post',
    // Research-specific vocabulary
    'endorser', 'endorsement', 'arxiv', 'paper', 'survey', 'canon',
    'inclusion', 'omission', 'co-author', 'coauthor', 'rebuttal',
    // Acquisition / pitch / brand vocabulary
    'pitch deck', 'pitch wedge', 'brief founder', 'brief the founder',
    // Affect words that indicate the agent is signalling stakes
    'opportunity', 'window closing', 'goldmine', 'vindication',
    'breakthrough', 'crisis', 'regression',
  ];

  return actionSignals.some(signal => text.includes(signal));
}
...
class MeshAgent {
   ...
   * @param {object} [opts.svafFieldWeights] — per-category SVAF weights
   ...
   * @param {Function} opts.reason — async (domainData, meshContext) => CAT7 categories | null
   * @param {Function} opts.remix — async (incomingCMB, meshContext) => CAT7 categories | null
  ...
  async _onCMBAccepted(entry) {
    ...
    const source = entry.source || recordCreatedBy(entry.cmb) || 'unknown';
    const focusText = entry.cmb?.categories?.focus?.text || entry.content || '';

    // Fix: only remix original domain observations, not remixes of remixes.
    // A CMB with lineage parents is itself a remix — re-remixing it creates
    // cascade storms across agents. Store it (SVAF accepted it) but don't
    // spend an LLM call to remix it.
    const lineage = recordLineage(entry.cmb) || entry.lineage;
    if (lineage?.parents?.length > 0) {
      console.log(`[${this._name}] accepted remix from ${source} — no re-remix`);
      return;
    }

    // Fix: skip anchor CMBs replayed on reconnect — they're historical context,
    // not new signals worth remixing.
    if (entry._anchor) {
      console.log(`[${this._name}] anchor from ${source} — no remix`);
      return;
    }
    ...
    const incoming = {
      ...
      categories: Object.fromEntries(
        Object.entries(categories).map(([k, v]) => [k, typeof v === 'object' ? v.text || JSON.stringify(v) : v])
      ),
    };

    const meshContext = this._meshContext();
    const remixCategories = await this._remix(incoming, meshContext);
    if (!remixCategories) return;

    // Extract agent meta — travels with CMB over wire
    let meta = remixCategories._meta || null;
    delete remixCategories._meta;

    // Auto-detect founderAction if not explicitly set
    if (!meta?.founderAction && detectFounderAction(remixCategories)) {
      meta = meta || {};
      meta.founderAction = true;
    }

    // Store with lineage (Section 14.2)
    const parent = {
      key: entry.key,
      lineage: entry.lineage || recordLineage(entry.cmb) || null,
    };

    const stored = this._node.remember(remixCategories, { parents: [parent], ...(meta ? { meta } : {}) });
    if (!stored) {
  ...
  async _checkDomain() {
    ...
    const meshContext = this._meshContext();
    const categories = await this._reason(result.data, meshContext);
    if (!categories) {
      console.log(`[${this._name}] reason() returned null — silent`);
      return;
    }

    // Content-based dedup — catch cases where fingerprint changes but LLM
    // produces substantially the same CMB (e.g. same mesh signals, same conclusion).
    const focusText = typeof categories.focus === 'string' ? categories.focus : '';
    if (this._state._lastFocus && focusText.slice(0, 100) === this._state._lastFocus) {
      console.log(`[${this._name}] duplicate focus — silent`);
      return;
    }
    if (focusText) this._state._lastFocus = focusText.slice(0, 100);

    // Extract agent meta (e.g. founderAction) — travels with CMB over wire
    let meta = categories._meta || null;
    delete categories._meta;

    // Auto-detect founderAction if not explicitly set
    if (!meta?.founderAction && detectFounderAction(categories)) {
      meta = meta || {};
      meta.founderAction = true;
    }

    // Original observation — no parents
    const stored = this._node.remember(categories, meta ? { meta } : undefined);
    if (!stored) {
      console.log(`[${this._name}] observation rejected (duplicate)`);
      return;
    }

    console.log(`[${this._name}] domain observation: ${categories.focus?.slice(0, 100)}`);
    this._save();
  }
  ...
  _meshContext() {
    return (this._node.recall('') || [])
      ...
      .slice(0, 5)
      .map(r => ({
        focus: r.cmb?.categories?.focus?.text || r.content || '',
