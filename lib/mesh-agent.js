 ...
 * for action-signal vocabulary. The wider field surface catches
 * high-quality CMBs that put the action signal in the commitment
 * or mood field rather than restating it in intent.
 *
...
class MeshAgent {
  ...
  async _onCMBAccepted(entry) {
    ...
    let meta = remixFields._meta || null;
    delete remixFields._meta;

    // Auto-detect founderAction if not explicitly set
    if (!meta?.founderAction && detectFounderAction(remixFields)) {
      meta = meta || {};
