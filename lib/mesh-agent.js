 ...
 * Checks intent and issue fields for action signals.
 * See MMP Section 11: Feedback Modulation.
 */
function detectFounderAction(fields) {
  const intent = (typeof fields.intent === 'string' ? fields.intent : '').toLowerCase();
  const issue = (typeof fields.issue === 'string' ? fields.issue : '').toLowerCase();
  const text = intent + ' ' + issue;
  ...
  const actionSignals = [
    ...
    'competitive', 'threat', 'urgent', 'blocker', 'blocked',
