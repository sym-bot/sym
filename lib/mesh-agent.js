 ...
 */
function detectFounderAction(fields) {
  const get = (k) => {
    const v = fields[k];
...
class MeshAgent {
  ...
  async _checkDomain() {
    ...
    const focusText = typeof fields.focus === 'string' ? fields.focus : '';
    ...
    let meta = fields._meta || null;
    delete fields._meta;

    // Auto-detect founderAction if not explicitly set
    if (!meta?.founderAction && detectFounderAction(fields)) {
      meta = meta || {};
      meta.founderAction = true;
    }
