...
class BonjourDiscovery extends Discovery {
  ...
  _startDnsSd() {
    ...
    this._dnssdBrowse = spawn('dns-sd', ['-B', '_sym._tcp'], { stdio: ['ignore', 'pipe', 'ignore'], windowsHide: true });
  ...
  _resolvePeer(instanceName) {
    const { spawn } = require('child_process');
    const identity = this._identity;
    const resolve = spawn('dns-sd', ['-L', instanceName, '_sym._tcp', 'local.'], { stdio: ['ignore', 'pipe', 'ignore'], windowsHide: true });
    let resolveBuffer = '';
  ...
  _startBonjourFallback() {
    ...
    // bonjour-service's 'up' fires once per service name — if the TCP
    // connection dies (idle timeout, network hiccup), there's no re-fire.
    // The reconnect timer re-emits 'peer-found' for cached peers that
    // are no longer in _peers, triggering _connectToPeer. The dedup
    // checks in the peer-found handler prevent double-connects.
    ...
    this._browser.on('up', (service) => {
      ...
      // Prefer IPv4 over IPv6 link-local. bonjour-service resolves
      // services as fe80::... (IPv6 link-local) which requires a scope
      // ID (%interface) for TCP connections — and bonjour-service
      // doesn't provide it. IPv4 always works cross-platform.
    ...
    // Reconnect timer: every 15s, re-emit peer-found for cached peers
    // that are no longer connected. The peer-found handler in node.js
    // checks _pendingBonjour and existing transports, so re-emitting
    // for already-connected peers is a harmless no-op.
