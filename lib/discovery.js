...
class BonjourDiscovery extends Discovery {
  ...
  async stop() {
    ...
    if (this._loopbackExitHook) {
      try { process.removeListener('exit', this._loopbackExitHook); } catch {}
      this._loopbackExitHook = null;
    }
  ...
  // (with a heartbeat) and scans the dir for live, same-group peers, dialing
  ...
  _startLoopbackRegistry() {
    try {
      ...
      // registration behind until a pid-liveness check filters it out. A
      // one-shot sync unlink on 'exit' (fires on normal completion and
      // process.exit()) makes the common case self-clean. Guarded so repeated
      // starts don't stack listeners; removed again in stop().
      if (!this._loopbackExitHook) {
        this._loopbackExitHook = () => {
          try { if (this._regFile) fs.unlinkSync(this._regFile); } catch {}
        };
        process.once('exit', this._loopbackExitHook);
      }
      this._writeEndpoint();
      this._scanLoopback();
  ...
  _scanLoopback() {
    ...
    for (const f of files) {
      ...
      // Group isolation — only mesh the same service type (MMP §5.8).
      if (rec.serviceType && rec.serviceType !== this._serviceType) continue;
      // Liveness: same-host pid check. signal 0 just tests existence; EPERM
      // means the process exists but isn't ours (still alive); ESRCH = dead.
      let alive = true;
      if (rec.pid) {
      ...
      if (!alive || !fresh) {
        if (!alive) { try { fs.unlinkSync(full); } catch {} } // GC dead entries
        continue;
      }
