...
class BonjourDiscovery extends Discovery {
  ...
  _startBonjourFallback() {
    ...
    this._browser.on('up', (service) => {
      ...
      // Both sides attempt connection. The old leader-election gate
      // (identity.nodeId < peerId) was a premature optimization that
      // made discovery fragile: if the initiating side had a stale
      // bonjour cache (old port from a prior restart), no connection
      // happened because the other side was gated from trying.
      // _createPeer already deduplicates transports (skips if bonjour
      // transport exists for this peer), so double-connect is harmless.
