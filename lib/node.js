...
class SymNode extends EventEmitter {
  ...
  constructor(opts = {}) {
    ...
    // The redundancy floor — δ_f^near below which a field is already in memory. Every other
    ...
    //
    // ⚠ THIS KEY MOVES THE ACTING GATE, NOT A SHADOW. Design C's binary cut is currently
    // DERIVED from the five-valued band rather than computed independently, so C's floor and
    // B's floor are the same number. Setting this to tune C therefore changes what the node
    // actually ADMITS — a shadow that moves the acting gate has stopped being a shadow, which
    // is the shadow-decides-nothing property violated by configuration rather than by code.
    // Until C becomes an independent cut on δ^near, treat this as a production gating change
    // and not as an experiment knob. When that lands, this warning expires: setting the key
    // will move only C, which is the whole point of being able to set it.
  ...
  async auditLineageTethers(opts = {}) {
    ...
    for (const key of keys) {
      ...
      if (!cmb || !cmb.fields || parents.length === 0) continue;
      report.audited++;
  ...
  remember(fields, opts = {}) {
    ...
    if (!opts.cmb) {
      ...
      opts.cmb = createCMB({
        ...
        createdBy: this.name,
        lineage,
        room: this._group ?? null,
        to: opts.to ?? null,
      });
      // No remix re-key. The v2 address is the Merkle root over the seven fieldKeys and is
    ...
    //
    // `undelivered` is still sound as written, and is the load-bearing field:
