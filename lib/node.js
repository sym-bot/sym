...
const {
  encode, DIM, createCMB, FIELD_WEIGHT_PROFILES,
  SVAFEvaluator, FrameHandler, WakeManager,
...
class SymNode extends EventEmitter {
  ...
  constructor(opts = {}) {
    ...
    // Synthesis delegate — called when xMesh insight arrives
  ...
  remember(content, opts = {}) {
    if (!opts.cmb) {
      opts.cmb = opts.fields
        ? createCMB({ fields: opts.fields, createdBy: this.name, sourceUrl: opts.sourceUrl })
        : createCMB({ rawText: content, createdBy: this.name });
    }
