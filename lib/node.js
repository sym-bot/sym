  ...
  async auditLineageTethers(opts = {}) {
    ...
    for (const key of keys) {
      ...
      const ev = await evaluateLineageTetherFromText({
        // EXTERNAL API of the pinned @sym-bot/core (0.9.3): its options are still named
        // remixFields/anchorFields. The vocabulary crosses this boundary when the renamed
        // core ships and the pin moves — dependency names are theirs until then.
        remixFields: cmb.categories, anchorFields: anchorCategories,
