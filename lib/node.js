...
class SymNode extends EventEmitter {
  ...
  async auditLineageTethers(opts = {}) {
    ...
    for (const key of keys) {
      ...
      if (anchor.resolvedFromStore) { anchorFields = anchor.categories; anchorKey = anchor.key; }
      if (!anchorFields && opts.fetch === true) {
        ...
        for (const k of (anc?.length ? anc : parents)) {
          ...
          if (hit?.cmb?.categories) { anchorFields = hit.cmb.categories; anchorKey = k; report.fetched++; break; }
        }
      }
      if (!anchorFields) { report.unchecked++; continue; }

      const ev = await evaluateLineageTetherFromText({
        remixFields: cmb.categories, anchorFields,
        categoryWeights: this._svafFieldWeights, guardedThreshold: this._svafGuardedThreshold,
      });
   ...
   * @param {object} categoryVerdicts - per-CAT7-field verdict map (Phase A output)
  ...
  aggregateAttestations(cmbKey) {
    ...
    const categories = Object.create(null);    // cat7 field -> { verdict -> weight }
  ...
  /// §5.8 optional `room` field. Extracted into a helper so the dedup
  ...
  _addPeer(peer) {
    ...
    // Handshake — per MMP Section 5.2 + §5.8 optional `room` field
