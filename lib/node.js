...
const { nodeDir, loadOrCreateIdentity, acquireIdentityLock, resolveAvailableName, log: logMsg } = require('./config');
...
const {
  ...
  signCMB, mintRemixKey, signAttestation, verifyAttestation, verifyAttestationRole,
...
class SymNode extends EventEmitter {
   ...
   * @param {boolean} [opts.autoSuffix=false] — on a same-host name collision, resolve to `<name>-2`/`-3`/… instead of throwing EIDENTITYLOCK, so co-resident processes coexist. Read `node.name` for the resolved value (`node.requestedName` keeps the original).
  ...
  constructor(opts = {}) {
    ...
    // opts.autoSuffix (default off): when two co-resident processes want the
    // same name — a duplicate dev agent, or sessions sharing a fixed
    // SYM_NODE_NAME — resolve to `<name>-2`/`-3`/… instead of hard-failing
    // with EIDENTITYLOCK. The name is resolved BEFORE identity/dir/lock are
    // derived below, so all three key off the final coexisting name. Default
    // stays strict (throw) so hosts that rely on the lock to detect a
    // double-launch keep that behavior. Hosts can read back `node.name` to
    // see the resolved value. The base is kept when free, held by a dead
    // process, or held by us (acquireIdentityLock reclaims stale/own locks).
    this.requestedName = opts.name;
    this.name = opts.autoSuffix ? resolveAvailableName(opts.name) : opts.name;
    ...
    // Adaptive integration timescale (the liquid substrate). When enabled, the SVAF
    // memory-decay timescale shortens after recent guarded/rejected verdicts and lengthens
    // when stable — the Level-1 content gate driving the Level-2 temporal timescale, instead
    // of a fixed-gain integrator. Off by default: tau stays at freshnessSeconds (no change).
    ...
    // Very old nodes used `memories/` (different field layout) — that still goes
    // through MemoryStore's field-mapped one-time migration.
    ...
    // autonomous per-field admission is observable, not just its admitted
   ...
   *
   * @param {string} key - Content-address key (cmb-/cmb1-).
   * @param {object} [opts]
  ...
  async auditLineageTethers(opts = {}) {
    const { resolveTetherAnchor, evaluateLineageTetherFromText, signTetherAttestation } = require('@sym-bot/core');
    ...
    for (const key of keys) {
      ...
      if (!entry) continue;
      const cmb = entry.cmb;
      const parents = cmb?.lineage?.parents;
      if (!cmb || !cmb.fields || !Array.isArray(parents) || parents.length === 0) continue;
      report.audited++;

      let anchorFields = null;
      let anchorKey = null;
      const anchor = resolveTetherAnchor(cmb, (k) => (k === entry.key ? undefined : this._store.get(k)));
      if (anchor.resolvedFromStore) { anchorFields = anchor.fields; anchorKey = anchor.key; }
      if (!anchorFields && opts.fetch === true) {
        for (const k of (cmb.lineage.ancestors?.length ? cmb.lineage.ancestors : parents)) {
          const hit = await this.fetchCMB(k, { timeoutMs: opts.timeoutMs ?? 3000 });
          if (hit?.cmb?.fields) { anchorFields = hit.cmb.fields; anchorKey = k; report.fetched++; break; }
        }
      }
      if (!anchorFields) { report.unchecked++; continue; }

      const ev = await evaluateLineageTetherFromText({
        remixFields: cmb.fields, anchorFields,
        fieldWeights: this._svafFieldWeights, guardedThreshold: this._svafGuardedThreshold,
      });
      ...
      try {
        cmb.tether = signTetherAttestation({
          of: cmb.key, anchor: anchorKey, kernelId: ev.kernelId, drift: ev.drift,
      ...
      if (ev.tethered) {
        ...
        report.failedFloor++;
        if (applySever) {
          const oldAncestors = cmb.lineage.ancestors || parents;
          cmb.lineage = null;
          entry.lineage = null;
   ...
   * authoritative, durable per-field gating record (MMP admission-attestation
   * layer). Binds the gated CMB (`of`), this node's identity (`by`), the roster,
   * the overall + per-field verdict, this node's CLAIMED lifecycle role, and the
   ...
   * @param {object} fieldVerdicts - per-CAT7-field verdict map (Phase A output)
   * @param {string} method - 'neural' | 'heuristic'
   * @private
   */
  _buildAdmissionAttestation(of, verdict, fieldVerdicts, method) {
    const att = {
      of,
      by: this.nodeId,
      at: Date.now(),
    ...
    try {
      ...
      // trail for this CMB. _peers are same-group (mDNS service-type isolation), so
   ...
   * overall and per-CAT7-field verdicts are weighted tallies; `confidence` is the dominant
   * verdict's share of total weight.
   * @param {string} cmbKey — the gated CMB key (`of`)
   * @returns {{ of: string, total: number, weight: number, byRole: object, overall: object,
   *   fields: object, mismatches: object[], excluded: object[] }}
   */
  aggregateAttestations(cmbKey) {
    ...
    const fields = Object.create(null);    // cat7 field -> { verdict -> weight }
    ...
    for (const att of atts) {
      ...
      for (const [f, v] of Object.entries(att.fields || {})) {
        if (!fields[f]) fields[f] = Object.create(null);
        fields[f][v] = (fields[f][v] || 0) + w;
      }
    }
    ...
    return {
      ...
      fields: Object.fromEntries(Object.entries(fields).map(([f, t]) => [f, summarizeTally(t, weight)])),
      mismatches,
      excluded,
    };
  }
   ...
   * peer set is same-group by mDNS isolation, so this is roster-scoped. `exceptPeerId`
  ...
  /** Send a frame to every roster peer (same-group by mDNS isolation), optionally
  ...
  _ingestCheckpoint(cp, fromPeerId) {
    ...
    if (cp.roster && this._group && cp.roster !== this._group) return { ok: false, reason: 'roster-mismatch' };
  ...
  _ingestWitness(w, fromPeerId) {
    ...
    if (w.roster && this._group && w.roster !== this._group) return { ok: false, reason: 'roster-mismatch' };
  ...
  _ingestAttestation(att, fromPeerId, fromPeerName) {
    ...
    // Roster scope (defense in depth — gossip already stays in-group).
    if (att.roster && this._group && att.roster !== this._group) {
   ...
   *   alongside CAT7 fields. Rides the wire frame and the local store but is
   *   NOT part of cmbKey (CAT7 fields alone are the content-addressed
   ...
   *   Senders are responsible for ensuring CAT7 fields differ when payloads
  ...
  _pushInbox(entry) {
    ...
    this._inbox.push({
      ...
      from: entry.source || cmb.createdBy || 'unknown',
      content: entry.content || '',
      fields: cmb.fields || null,
      // Preserve the opaque payload (sibling of fields, NOT inside it). Without
      ...
      key: cmb.key || entry.key || null,
      receivedAt: Date.now(),
    });
    while (this._inbox.length > this._inboxMax) this._inbox.shift();
  }
  ...
  remember(fields, opts = {}) {
    if (!opts.cmb) {
      ...
      // MMP Section 15.7: enforce remix requires new domain data.
      ...
      if (lineage) {
        // §8.2.1 role dispatch: a lineage-bearing block is keyed under the
        // REMIX scheme (parents + author bound into the address). Receivers
        // recompute the key BY ROLE during signature verification, so a
        // root-keyed block that carries parents fails content verification
        // and is hard-rejected as forged — authored groundings never landed
        // on any peer until this re-key.
        opts.cmb.key = mintRemixKey(opts.cmb.fields, lineage.parents, this.name);
      }
      // Audience binding (§18.3.1): bind the authoring group and the directed
      // recipient into the CMB so the cmb1- signature covers them — a cross-group
      // or mis-directed replay then fails the receiver's audience check.
      opts.cmb.group = this._group;
      if (opts.to) opts.cmb.to = opts.to;
    ...
    // Per MMP Section 15.7: only original observations (no parents) count
    ...
    for (const [peerId, peer] of targets) {
      ...
      peer.transport.send({
        ...
        // receiver can distinguish a peer-bound (directed) send from a group-bound
   ...
   * per-field admission — INCLUDING rejections, which the memory store drops.
   * @private
   */
  _recordDecision(rec) {
    if (!rec) return;
   ...
   * per-field admission (admit AND reject). Newest-first.
...
function migrateStores(nodesRoot = path.join(require('os').homedir(), '.sym', 'nodes')) {
  let migrated = 0;
  let dirs;
