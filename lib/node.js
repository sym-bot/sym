...
const { AttestationStore } = require('./attestation-store');
...
class SymNode extends EventEmitter {
  ...
  constructor(opts = {}) {
    ...
    // (admit/guard/redundant/reject), and (in the gossip phase) those received from
    // roster peers. Indexed by gated-CMB-key (the audit trail) and by attester chain
    // (omission-evidence). The queryable backbone of the cross-mesh audit trail.
