...
class SymNode extends EventEmitter {
  ...
  _roomAdmission(peerId, handshakeMsg) {
    ...
    // partition, silent on both sides. The field arrived in 0.11.0 as a rename of
    // `group`, which nothing reads today, so pre-0.11.0 JS nodes are in the same set.
    //
    // Admitting on absence leaves a real hole: a relay token authenticates to the
    // RELAY, not to a room, so an absent claim arriving over the relay is unfiltered.
    // That hole is exactly as open in 0.12.3 as here, and closing it by disconnecting
    // every phone is the wrong trade. The admission is logged so the population is
    // countable, and this tightens the day sym-swift sends a room.
