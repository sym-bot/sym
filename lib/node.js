...
class SymNode extends EventEmitter {
  ...
  constructor(opts = {}) {
    ...
    // grant sigs against the shared handshake key map; persisted under the node dir.
    this._roleGrants = new RoleGrantStore({
      anchor: this._anchor,
      keys: this._peerIdentityKeys,
  ...
  _ingestCheckpoint(cp, fromPeerId) {
    ...
    const key = this._peerIdentityKeys?.get(cp.by);
  ...
  _ingestWitness(w, fromPeerId) {
    ...
    const key = this._peerIdentityKeys?.get(w.by);
  ...
  _ingestAttestation(att, fromPeerId, fromPeerName) {
    ...
    const attesterKey = this._peerIdentityKeys?.get(att.by);
  ...
  async start() {
    ...
    this._discovery.on('inbound-connection', (transport, peerId, peerName, handshakeMsg) => {
      ...
      if (handshakeMsg.publicKey) {
        this._peerIdentityKeys.set(peerId, handshakeMsg.publicKey);
      }
