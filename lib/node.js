...
const { recordKey, recordCreatedBy, recordLineage, recordParents, setRecordLineage } = require('./record');
...
const { AttestationStore, chainHash } = require('./attestation-store');
...
const {
  ...
  WakeManager,
  e2eGenerateKeyPair, e2eDeriveSharedSecret, encryptCategories,
...
/**
 * NEVER IN THE CLEAR THROUGH THE RELAY. A peer reached only over the relay with no shared
 * secret (its handshake carried no E2E key, or derivation failed) used to receive the CMB's
 * categories in plaintext, through a server that promises it cannot read them. Founder,
 * 2026-09-05: refuse instead — the peer gets nothing from us, and the refusal is said once per
 * peer (log + `peers()` carries `e2e` and `clearRefused`) so silence has a reason. LAN transports
 * are unaffected: a peer on the local network still receives the frame. Plain functions: the
 * node already has a boolean field named `_relayOnly` (an option), so these live outside it.
 */
function peerReachedOnlyOverRelay(peer) {
  return !!(peer && peer.transports) && peer.transport === peer.transports.get('relay');
}
function refuseClearThroughRelay(node, peerId, peer) {
  if (!node._clearRefused) node._clearRefused = new Set();
  if (node._clearRefused.has(peerId)) return;
  node._clearRefused.add(peerId);
  if (typeof node._log === 'function') {
    node._log(`Refused to send in the clear through the relay to ${(peer && peer.name) || String(peerId).slice(0, 8)} — that peer presented no encryption key, so nothing is sent to it over the relay until it does`);
  }
}
...
class SymNode extends EventEmitter {
  ...
  constructor(opts = {}) {
    ...
    //
    // NO `??` DEFAULT HERE, deliberately. Left undefined, core's svaf-heuristic applies its
    ...
    this._attestations = new AttestationStore({ dir: path.join(this._dir, 'attestations') });
    ...
    // E2E encryption — X25519 keypair for CMB category encryption
    this._e2eKeyPair = this._loadOrCreateE2EKeyPair();
    this._peerSharedSecrets = new Map();

    // CMB authentication — peers' Ed25519 identity public keys (from handshake,
    // base64url). Used to verify the signature on every inbound signed CMB.
    this._peerIdentityKeys = new Map();
    ...
    // given time, with authority flowing only along anchor-rooted chains. Verifies
    // grant sigs against the roster registry (which a rooted grant can populate with the
    // grantee's vouched key); persisted under the node dir.
  ...
  /**
   * Load or create a persistent X25519 keypair for E2E category encryption.
  ...
  /**
   * Encrypt CMB categories for a specific peer. Returns a modified CMB copy
   * with categories replaced by ciphertext and E2E metadata attached.
   * @param {object} cmb — original CMB
   * @param {Buffer} sharedSecret — 32-byte shared secret
   * @returns {object} — CMB with encrypted categories
   * @private
   */
  _encryptCMBForPeer(cmb, sharedSecret) {
    if (!cmb || !cmb.categories || typeof cmb.categories !== 'object') return cmb;
    const { ciphertext, nonce } = encryptCategories(cmb.categories, sharedSecret);
    return {
      ...cmb,
      categories: ciphertext,
      _e2e: { nonce },
    };
  }
  ...
  _resolvedRole() {
    ...
    return this._roleGrants.resolveRole(this.nodeId, Date.now());
  }

  /** Resolve any node's role at a time (default now) from this node's grant chain. */
  resolveRole(nodeId, at = Date.now()) {
    return this._roleGrants.resolveRole(nodeId, at);
  }

  /**
   * Learn a peer's identity key from a direct authenticated handshake. Pins it both in
   * the legacy CMB-verification map and in the roster registry at 'handshake' strength
   * (so it outranks any weaker grant-vouched binding).
   * @private
   */
  _pinPeerKey(nodeId, keyB64url) {
    if (!nodeId || !keyB64url) return;
    this._peerIdentityKeys.set(nodeId, keyB64url);
    this._roster.pin(nodeId, keyB64url, 'handshake');
  }

  /**
   * Resolve a node's identity public key for signature verification. Consults the roster
   * registry first (covers relayed, grant-vouched, and persisted bindings — peers we may
   * never have directly met), falling back to the direct-handshake map.
   * @private
   */
  _identityKey(nodeId) {
    return this._roster.get(nodeId) || this._peerIdentityKeys?.get(nodeId);
  }
  ...
  async auditLineageTethers(opts = {}) {
    const { resolveTetherAnchor, evaluateLineageTetherFromText, signTetherAttestation } = require('./core');
    ...
    for (const key of keys) {
      ...
      if (!entry) continue;
      const cmb = entry.cmb;
      const parents = recordParents(cmb);
      ...
      if (!anchorCategories && opts.fetch === true) {
        const anc = recordLineage(cmb)?.ancestors;
      ...
      try {
        cmb.tether = signTetherAttestation({
      ...
      if (ev.tethered) {
        ...
        report.failedFloor++;
        if (applySever) {
          const oldAncestors = recordLineage(cmb)?.ancestors || parents;
          setRecordLineage(cmb, null);   // clears the section the record ACTUALLY carries
          entry.lineage = null;
          for (const a of oldAncestors) this._store._index.byAncestor?.get(a)?.delete(entry.key);
          report.severed++;
    ...
    return await new Promise((resolve) => {
      ...
      for (const [peerId, peer] of this._peers) {
        if (peer.transport?.send) {
          asked.add(peerId);
          peer.transport.send({ type: 'cmb-fetch', reqId, key, from: this.nodeId, timestamp: Date.now() });
        }
      }
   ...
   * identity key, records it, and gossips it to the roster. The grant only confers
   * authority on the grantee if THIS node's own resolved role outranks-or-equals the
   * conferred role and roots at the anchor — enforced at resolve time, so a grant
   * this node isn't entitled to make is stored but inert.
   * @param {string} granteeNodeId
   * @param {'validator'|'anchor'} role
   * @returns {object|null} the signed grant, or null
   */
  grantRole(granteeNodeId, role) {
    return this._emitGrant({ type: 'role-grant', grantee: granteeNodeId, role });
  }
  ...
  _emitGrant(partial) {
    ...
    // Vouch for the grantee's nodeId↔key binding when we know its key (from a direct
    // handshake): bound into the signed payload, this lets nodes that never met the
    // grantee learn its key from this rooted grant. Revokes carry no key.
    if (g.type === 'role-grant' && !g.granteeKey) {
      const k = this._identityKey(g.grantee);
      if (k) g.granteeKey = k;
    }
    try {
      signGrant(g, this._identity.privateKey);
      this._roleGrants.record(g);
  ...
  /**
   * Ingest a role-grant/revoke gossiped by a roster peer: the store verifies the
   * grantor's signature against its authenticated key (anchor pinned), records it,
   * and we relay-once. Authority (whether the grantor was entitled) is resolved
   * lazily, so an unentitled grant is stored but confers nothing.
   * @private
   */
  _ingestRoleGrant(g, fromPeerId) {
    if (!g) return { ok: false, reason: 'malformed' };
    const r = this._roleGrants.record(g);
    if (r.stored) this._gossipToRoster({ type: g.type, grant: g }, fromPeerId);
  ...
  _ingestCheckpoint(cp, fromPeerId) {
    if (!cp || !cp.by || cp.upto_seq === undefined || !cp.root) return { ok: false, reason: 'malformed' };
  ...
  _ingestWitness(w, fromPeerId) {
    if (!w || !w.attester || w.upto_seq === undefined || !w.root || !w.by) return { ok: false, reason: 'malformed' };
  ...
  /**
   * Ingest an attestation received from a roster peer: roster-scope check, verify the
   * attester's Ed25519 signature against its authenticated identity key, rate-limit
   * per (of,by), record, and relay-once on first sight (epidemic spread). Returns the
  ...
  _ingestAttestation(att, fromPeerId, fromPeerName) {
    if (!att || !att.sig || !att.of || !att.by) return { ok: false, reason: 'malformed' };
    ...
    if (r.stored) this._gossipAttestation(att, fromPeerId);
    return { ok: r.stored, reason: r.reason };
  }
  ...
  shareWithPeers(content, opts = {}) {
    ...
    for (const [peerId, peer] of this._peers) {
      ...
      if (sharedSecret) {
        ...
        this._log(`E2E encrypted categories for peer ${peerId.slice(0, 8)}`);
      } else if (peerReachedOnlyOverRelay(peer)) {
        refuseClearThroughRelay(this, peerId, peer);
        continue;
      }
      peer.transport.send({
        type: 'cmb',
    ...
    return { key: recordKey(cmb), content, cmb, timestamp: frame.timestamp };
  }
  ...
  async start() {
    ...
    this._discovery.on('inbound-connection', (transport, peerId, peerName, handshakeMsg) => {
      ...
      if (existingPeer && existingPeer.transports.has('bonjour')) {
        ...
        if (!stalePrior) {
          // Determine prior direction. The peer's `isOutbound` category reflects
      ...
      // Store the connector's Ed25519 identity key (this first handshake arrives
      // via discovery, not the frame-handler) so we can verify its signed CMBs.
      if (handshakeMsg.publicKey) {
        this._pinPeerKey(peerId, handshakeMsg.publicKey);
      }
      ...
      const admission = this._roomAdmission(peerId, handshakeMsg);
      if (!admission.admit) {
        this._log(`Refused ${peerName} into '${this._room}': ${admission.reason}`);
  ...
  _pushInbox(entry) {
    ...
    this._inbox.push({
      seq: ++this._inboxSeq,
      id: `in${String(this._inboxSeq).padStart(4, '0')}`,
      from: entry.source || recordCreatedBy(cmb) || 'unknown',
    ...
    while (this._inbox.length > this._inboxMax) {
      ...
      if (oldest && oldest.seq > this._inboxCursor) {
   ...
   *            oldestUndrainedAt:number|null, neverDrained:boolean, dropped:number}}
   */
  inboxStatus() {
    const undrained = this._inbox.filter((m) => m.seq > this._inboxCursor);
    return {
      seq: this._inboxSeq,
   ...
   * @param {number}  [opts.limit=50]   — max messages (oldest-first).
  ...
  inbox(opts = {}) {
    ...
    if (!opts.peek && slice.length) {
      this._inboxCursor = Math.max(this._inboxCursor, slice[slice.length - 1].seq);
  ...
  remember(categories, opts = {}) {
    ...
    if (!opts.cmb) {
      ...
      opts.cmb = createCMB({
        ...
        to: opts.to ?? null,
        // Reader-first gated (lib/emit-policy): populates the v2.0 preimage fields — and so signs
        // under mmp-sig-v2.0 — only once MMP_EMIT_V2 is flipped on after reader deployment.
        emitV2: MMP_EMIT_V2,
      ...
      try {
        signCMB(opts.cmb, this._identity.privateKey);
      } catch (err) {
        this._log(`CMB signing failed: ${err.message}`);
      }
    }
    ...
    if (mintKey && this._head && mintKey === this._head) {
      ...
      if (opts.cmb.metadata) opts.cmb.metadata.lineage = null;
      ...
      return { key: mintKey, cmb: opts.cmb, collapsed: true };
    }
    ...
    for (const [peerId, peer] of targets) {
      ...
      if (sharedSecret && baseCmb) {
        ...
        this._log(`E2E encrypted categories for peer ${peerId.slice(0, 8)}`);
      } else if (baseCmb && peerReachedOnlyOverRelay(peer)) {
        refuseClearThroughRelay(this, peerId, peer);
        continue;
      }
      peer.transport.send({
        ...
        cmb,
        // MMP §4.4.4 targeted CMB: mark the wire frame with the addressee so the
        // receiver can distinguish a peer-bound (directed) send from a room-bound
        // broadcast. Directed CMBs are surfaced unconditionally on receipt; SVAF then
        // governs memory admission only. Broadcasts stay fully SVAF-gated.
        ...(opts.to ? { to: opts.to, directed: true } : {}),
      });
      shared++;
    }
    ...
    if (entry.delivery.undelivered) {
      ...
      this._log(`UNDELIVERED (directed): ${opts.to.slice(0, 8)} not connected — stored locally, NOT sent`);
      this.emit('metric', { type: 'cmb-undelivered', to: opts.to, key: entry.key });
    }
  ...
  peers() {
    ...
    for (const [id, peer] of this._peers) {
      ...
      result.push({
        ...
        e2e: this._peerSharedSecrets.has(id),
        clearRefused: !!(this._clearRefused && this._clearRefused.has(id)),
      });
    }
    return result;
  }
  ...
  /// §5.8 optional `room` category. Extracted into a helper so the dedup
  ...
  _buildHandshake() {
    return {
      ...
      extensions: [],
      room: this._room,
      roomGrant: this._roomGrant || undefined,
  ...
  /**
   * MAY THIS PEER BE IN THIS ROOM? The single admission decision, called from both
   * directions — the inbound handshake we receive before admitting, and the handshake an
   * outbound-dialled peer sends back after we connect.
   *
   * BEFORE THIS, NOTHING COMPARED THE ROOM AT ALL. The handshake has always carried
   * `room` and bound it into the signed transcript, but no receiver ever looked: a peer
   * announcing any room joined the peer set, and LAN isolation was a property of the
   * mDNS browse filter rather than of any check. That is why a crew could meet another
   * tenant's commissions in a room neither had chosen to share.
   *
   * FAIL-CLOSED ONLY WHERE SOMEONE ASKED FOR IT. A room with no owner is OPEN and this
   * returns admit for exactly the traffic it always admitted, so upgrade day changes
   * nothing anywhere. Ownership is opt-in, and the point of an owner is not to shrink the
   * room: a mesh whose advantage depends on genuinely distinct nodes needs foreign peers
   * to be welcome, and a grant is what lets an owner say yes to one on purpose.
   *
   * Direction-agnostic by construction: the loopback tie-break means the stranger dials
   * US in half of all nodeId orderings, so a check on only the path we initiate is dead
   * code for those pairs.
   *
   * @returns {{ admit: boolean, reason?: string }}
   */
  _roomAdmission(peerId, handshakeMsg) {
    ...
    // square. Collapsing the two with `|| 'default'` turns silence into a positive
    // claim, which then reads as a mismatch against any named room. §5.8 requires a
    // receiver to close on MISMATCH; a peer that made no claim has not mismatched.
    //
    // Not hypothetical: sym-swift's SymFrame.handshake takes no room parameter and
    // sets no room key, so EVERY shipped MeloTune and MeloMove device sends a
    // handshake without one. Under the collapse, a node in any named room closed the
    // transport on the whole iOS fleet — not a tightening of isolation but a hard
    // partition, silent on both sides. The handshake carried no room at all through
    // 0.11.0; the field arrived in 0.11.1, so every JS node at 0.11.0 or older is in
    // the absent set too. (What 0.11.1 renamed from `group` was emit.js's field on the
    // CMB RECORD — a different object from this frame. Both halves of the earlier note
    // here were wrong, and it is corrected rather than quietly dropped.)
    //
    // On the relay path, narrower than it looks: a relay token is the security
    // boundary and fixes the CHANNEL from server-held state, while the room is a
    // client-declared PARTITION inside it that the relay honours — delivery, roster and
    // departures are scoped by it, and an undeclared room is its own unnamed partition
    // rather than the global mesh. sym-swift already sends `room` in relay-auth even
    // though its handshake carries none, so devices are partitioned correctly there.
    // The residual is a design position, not a defect: inside a channel a peer already
    // holds a token for, the room is addressing and not a security boundary.
    //
    // The admission is logged so the population is countable, and this tightens the day
    // sym-swift sends a room in the handshake.
    const claimed = handshakeMsg && handshakeMsg.room;
    if (!claimed) {
      // State the OBSERVATION, not the inference. "predates room comparison" was
      // true while the absent set was pre-0.11.1 nodes and the iOS fleet, and
      // 0.13.1 added a second population to this same line: an emit() caller that
      // named no room predates nothing — it is current code exercising a
      // documented default, and that is a caller-side choice which may be fine
      // forever. The two need opposite responses (wait out and upgrade, versus
      // leave alone), and this line exists to be COUNTED before tightening the
      // rule. A count that mixes them can never reach zero, so it would either
      // hold the tightening forever or get overridden on a hunch. Absence is
      // byte-identical on the wire, so the cause is not observable here at all —
      // report what was seen and leave the why to whoever reads the count.
      this._log(`Admitting ${peerId} into '${this._room}': no room claimed`);
      return { admit: true };
    }
    if (claimed !== this._room) return { admit: false, reason: `room-mismatch (claims '${claimed}', this node is in '${this._room}')` };

    const owner = this._roomOwners.ownerOf(this._room);
    if (!owner) return { admit: true };                       // ungated — unchanged

    const granteeId = (handshakeMsg && handshakeMsg.nodeId) || peerId;
    // The owner needs no grant to be in the room it owns — without this the owner locks
    // itself out of its own room the moment it gates it.
    if (granteeId === owner.nodeId) return { admit: true };

    const grant = handshakeMsg && handshakeMsg.roomGrant;
    if (!grant) return { admit: false, reason: 'no room-join grant presented for a gated room' };
    const v = verifyRoomGrant(grant, owner.publicKey, { room: this._room, grantee: granteeId });
  ...
  _addPeer(peer) {
    ...
    // Handshake — per MMP Section 5.2 + §5.8 optional `room` category
    ...
    if (now - lastSent < 60000) {
      ...
      // Anchors must be this node's OWN recent emissions (peerId == null). Forwarding
      // CMBs we merely received from other peers re-gossips them across the mesh
      // (A->B->C->A) and is the amplifier behind cross-node echo storms (MMP §14): a
      // peer learns each node's state from that node directly, so own-origin anchors
      // are sufficient context. Pull a wider window, then keep the 5 most-recent own.
      const recentAnchors = this._store.recent(50).filter(a => a.cmb && a.peerId == null).slice(0, 5);
      for (const anchor of recentAnchors) {
        ...
        if (sharedSecret) {
          ...
          refuseClearThroughRelay(this, peer.peerId, peer);
          break;
        }
        peer.transport.send({
          type: 'cmb',
...
function _nodeIsLive(nodeDir) {
  const { readLockFile, lockIsHeldByLiveProcess } = require('./config');
  return lockIsHeldByLiveProcess(readLockFile(path.join(nodeDir, 'lock.pid')));
}
...
function migrateStores(nodesRoot = path.join(require('./core/state-root').SYM_STATE_DIR, 'nodes')) {
  ...
  for (const d of dirs) {
    ...
    const nodeDir = path.join(nodesRoot, d.name);
    if (_nodeIsLive(nodeDir)) continue; // live node self-migrates on restart
    const meshmem = path.join(nodeDir, 'meshmem');
    const cmbs = path.join(nodeDir, 'cmbs');
    if (fs.existsSync(meshmem) && !fs.existsSync(cmbs)) {
      try { fs.renameSync(meshmem, cmbs); migrated++; } catch { /* leave it */ }
    }
  }
  return migrated;
}
