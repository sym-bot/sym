...
/**
 * llm-reason — invoke LLM and extract structured CAT7 output.
 *
 * Single public function: invoke({ agentDir, agentName, prompt, ... })
 * Prompt construction is the agent's responsibility.
 * The SDK handles: invocation, output parsing, CAT7 validation.
 *
 * Three providers:
 *   - 'openai'    — POST api.openai.com (default; works with any
 *                   OpenAI-compatible endpoint: Ollama, vLLM, etc.)
 *   - 'anthropic' — POST api.anthropic.com (Anthropic Messages API)
 *   - 'cli'       — Spawn `claude` CLI as subprocess. Gives the agent
 *                   full Claude Code tool access (Read, Bash, WebFetch,
 *                   Skill, etc.) and auto-loads SKILL.md / CLAUDE.md
 *                   from the agent dir. Uses local Claude Code auth —
 *                   no API key needed. See lib/llm-cli.js.
 *
 * Provider selection (in priority order):
 *   1. opts.provider                — per-call override
 *   2. process.env.SYM_LLM_PROVIDER — global default
 *   3. 'openai'                     — fallback
 *
 * Other env vars:
 *   SYM_LLM_API_KEY   — API key (or OPENAI_API_KEY / ANTHROPIC_API_KEY)
 *   SYM_LLM_MODEL     — model name (cli: opus|sonnet|haiku, others: full id)
 *   SYM_LLM_BASE_URL  — custom endpoint (openai/anthropic only)
 *
...
const { callClaudeCLI } = require('./llm-cli');
...
function isValidCAT7(obj) {
  ...
  for (const category of CAT7_FIELDS) {
...
function loadRolePrompt(agentDir) {
  ...
  return 'You are a mesh agent. Produce a CMB with 7 CAT7 categories.';
}
...
function getProviderConfig(opts) {
  ...
  if (provider === 'cli' || provider === 'claude-cli') {
    return {
      provider: 'cli',
      model: opts.model || process.env.SYM_LLM_MODEL || process.env.CLAUDE_AGENT_MODEL || 'sonnet',
      cwd: opts.agentDir,
      addDirs: Array.isArray(opts.addDirs) ? opts.addDirs : [],
      allowedTools: opts.allowedTools,           // optional whitelist
      permissionMode: opts.permissionMode,       // default 'bypassPermissions' inside callClaudeCLI
 ...
 * @returns {Promise<object|null>} CAT7 categories or null on failure
 */
async function invoke(opts) {
  const rolePrompt = loadRolePrompt(opts.agentDir);
  const config = getProviderConfig(opts);

  // The CLI provider uses local Claude Code auth — no API key check.
  // The HTTP providers need a key.
  if (config.provider !== 'cli' && !config.apiKey) {
    console.error(`[llm] ${opts.agentName} no API key configured. Set SYM_LLM_API_KEY, OPENAI_API_KEY, or ANTHROPIC_API_KEY`);
    return null;
  }

  // Suffix appended to every prompt. Reminds the model of the canonical
  // CAT7 schema AND the optional _meta tagging category, so agents whose
  // role definition requests _meta.founderAction (per MMP Section 11)
  // actually get it in the output. Without this, the model often emits
  // just the 7 categories and the founderAction signal is lost — which
  // forces sym.day to fall back to keyword scraping, which misses any
  // CMB that uses non-cliché vocabulary.
  const userPrompt = opts.prompt +
    '\n\nReturn a JSON object with the 7 CAT7 categories and an optional _meta tag:' +
    '\n{"focus":"...","issue":"...","intent":"...","motivation":"...","commitment":"...","perspective":"...","mood":{"text":"...","valence":-1.0..1.0,"arousal":-1.0..1.0},' +
    '\n "_meta":{"founderAction":true|false,"urgency":"critical|normal|low","reason":"one line"}}' +
    '\nSet _meta.founderAction=true ONLY if your role definition\'s rules for it are satisfied. Otherwise set it false or omit _meta.';

  try {
    let callFn;
    if (config.provider === 'cli') {
      // CLI calls don't need withRetry — the subprocess either succeeds
      // or fails fast. Retrying a CLI call typically just wastes a
      // minute on the same error.
      callFn = () => callClaudeCLI(config, rolePrompt, userPrompt);
    } else if (config.provider === 'anthropic') {
      callFn = () => callAnthropic(config, rolePrompt, userPrompt);
    } else {
      callFn = () => callOpenAI(config, rolePrompt, userPrompt);
    }

    const result = config.provider === 'cli' ? await callFn() : await withRetry(callFn);
...
/**
 * Free-form completion — same providers as invoke(), but returns the raw
 * text instead of extracting CAT7. Used by `sym ask` to synthesize a single
 * answer from the contributions the mesh has gathered.
 *
 * @param {object} opts
 * @param {string} opts.systemPrompt — system / role prompt
 * @param {string} opts.prompt       — user prompt
 * @param {function} [opts.onUsage]  — token usage callback
 * @returns {Promise<{text:string, model:string, provider:string}>}
 * @throws {Error} code 'NO_PROVIDER' if no API key / CLI provider is configured
 */
async function complete(opts) {
  ...
  let callFn;
  if (config.provider === 'cli') {
    callFn = () => callClaudeCLI(config, systemPrompt, userPrompt);
  } else if (config.provider === 'anthropic') {
    callFn = () => callAnthropic(config, systemPrompt, userPrompt);
  } else {
    callFn = () => callOpenAI(config, systemPrompt, userPrompt);
  }
...
/**
 * Whether an LLM provider is configured (HTTP key present, or CLI provider).
 * Lets callers choose synthesis vs. raw output without making a network call.
 */
function hasProvider(opts = {}) {
  const config = getProviderConfig(opts);
  return config.provider === 'cli' || !!config.apiKey;
}

module.exports = { invoke, complete, hasProvider, getProviderConfig, loadRolePrompt, extractCAT7, isValidCAT7 };
