...
class AttestationStore {
  ...
  /** Record an (already-verified) checkpoint, keyed by attester + upto_seq. Idempotent. */
  recordCheckpoint(cp) {
    ...
    if (existing && existing.sig === cp.sig) return { stored: false, reason: 'duplicate' };
  ...
  recordWitness(w) {
    ...
    if (m.has(w.by) && m.get(w.by).sig === w.sig) return { stored: false, reason: 'duplicate' };
    m.set(w.by, w);
  ...
  _replay(file, fn) {
    let text;
    try { text = fs.readFileSync(path.join(this._dir, file), 'utf8'); }
