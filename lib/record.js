 ...
 * metadata section is what the signature binds; a flat top-level category on a record that also has
 * metadata is either a leftover or something a relayer stapled on. Reading legacy-first would
 * mean a current record could be described by an unsigned category — the same defect class as
 ...
 * anything — it adds a null top-level category that nothing reads while the real lineage stays in
